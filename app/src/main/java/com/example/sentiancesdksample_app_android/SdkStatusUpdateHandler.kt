...
class SdkStatusUpdateHandler internal constructor(private val mContext: Context) :
    OnSdkStatusUpdateHandler {
    override fun onSdkStatusUpdate(status: SdkStatus) {
        Log.i(TAG, "SDK status updated: $status")
        ...
        LocalBroadcastManager.getInstance(mContext).sendBroadcast(
            Intent(
                ACTION_SENTIANCE_STATUS_UPDATE
            )
        )
        checkStatus(status)
    }

    private fun checkStatus(status: SdkStatus) {
    ...
    private fun checkDetectionIssues(status: SdkStatus) {
        println("status =>$status")

//        its not avaiable anymore
...
//        }
    ...
    private fun checkQuotas(status: SdkStatus) {
    ...
    companion object {
        const val ACTION_SENTIANCE_STATUS_UPDATE = "ACTION_SENTIANCE_STATUS_UPDATE"
        private const val TAG = "SdkStatusUpdate"
    }
}